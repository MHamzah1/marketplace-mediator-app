...
function getErrorMessage(error: unknown) {
  ...
  return "Profil belum bisa disimpan. Silakan coba lagi.";
}
...
export default function EditProfileScreen() {
  ...
  useEffect(() => {
    ...
    if (user.dateOfBirth) {
      setCalendarMonth(getInitialCalendarMonth(user.dateOfBirth));
    }
  }, [user]);

  const existingPhotoUri = useMemo(
    () => resolveImageUrl(user?.profilePhoto || user?.profileImage),
  ...
  const isAtMinCalendarMonth =
    calendarMonth.getFullYear() === MIN_BIRTH_YEAR &&
  ...
  const handleSave = async () => {
    ...
    if (!fullName.trim() || !normalizedPhone) {
      Alert.alert(
        "Data Belum Lengkap",
        "Mohon isi nama lengkap dan nomor telepon.",
      );
      return;
    }
    ...
    try {
      ...
      Alert.alert("Profil Diperbarui", "Data profil Anda berhasil disimpan.", [
        { text: "OK", onPress: () => router.back() },
      ]);
    } catch (error) {
      Alert.alert("Gagal Menyimpan", getErrorMessage(error));
    } finally {
      setSaving(false);
    }
  };
      ...
      >
        <View style={styles.container}>
          ...
          <View style={styles.form}>
            ...
            <Text style={styles.sectionTitle}>Lokasi & Bio</Text>
            <AuthInputField
              label="Lokasi"

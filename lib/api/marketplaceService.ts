...
export async function fetchListings(params: FilterParams = {}): Promise<PaginatedResponse<Listing>> {
  const response = await axiosInstance.get('/marketplace/listings', { params });
  return response.data;
}

// Get featured listings (public)
export async function fetchFeaturedListings(limit = 10, category?: string): Promise<{ data: Listing[]; total: number }> {
...
export async function fetchListingDetail(id: string): Promise<ApiResponse<Listing>> {
...
export async function fetchSellerProfile(id: string): Promise<ApiResponse<SellerProfile>> {
...
export async function deleteListing(id: string): Promise<{ message: string; deletedId: string }> {
